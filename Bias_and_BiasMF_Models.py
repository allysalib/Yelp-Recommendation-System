...
def main():
    ...
    for d in damping_values:
        ...
        for train_val, test in partition_users(ratings_data, N_SPLIT, SampleFrac(FRAC_SPLIT, rng_spec=13), rng_spec=13):
            ...
            for train, val in partition_users(train_val, N_SPLIT, SampleFrac(FRAC_SPLIT, rng_spec=13), rng_spec=13):
                ...
                bias_validation_evals_list.append([(count-1), d, evals_full])
    ...
    bias_validation_evals_splits = bias_validation_evals_df.groupby(['damping_factor', 'split']).agg({'recip_rank': ['mean', 'std'], 'precision@10': ['mean', 'std'], 'precision@100': ['mean', 'std'], 'recall@10': ['mean', 'std'], 'recall@100': ['mean', 'std'], 'ndcg': ['mean', 'std'], })
    ...
    for i in best_bias_models:
        ...
        evals_100 = results_100recs[['precision', 'recall']].rename(columns={"precision": "precision@100", "recall": "recall@100"})
        evals_full = pd.concat([evals_10, evals_100], axis=1, sort=False)
        bias_test_evals_list.append([count, evals_full])
    ...
    bias_test_evals_splits = bias_test_evals_df.groupby(['split']).agg({'recip_rank': ['mean', 'std'], 'precision@10': ['mean', 'std'], 'precision@100': ['mean', 'std'], 'recall@10': ['mean', 'std'], 'recall@100': ['mean', 'std'], 'ndcg': ['mean', 'std'], })
    ...
    bias_test_evals_aggregated = bias_test_evals_df.agg({'recip_rank': ['mean', 'std'], 'precision@10': ['mean', 'std'], 'precision@100': ['mean', 'std'], 'recall@10': ['mean', 'std'], 'recall@100': ['mean', 'std'], 'ndcg': ['mean', 'std'], })
    print("Aggregated Bias models test evaluations for best parameters:")
    print(bias_test_evals_aggregated)

    regularization_values = [0, 0.1, 0.5, 0.75, 1]
